...
for item in matriz:
    ...
    nueva_fila = [
        enunciado,
        respuestas["respuesta_1"],
        respuestas["respuesta_2"],
        respuestas["respuesta_3"],
        respuestas["respuesta_4"],
        respuestas["respuesta_correcta"],
        0,  # veces_preguntada
        0,  # aciertos
        0   # fallos
    ]
    matriz_sin_diccionario.append(nueva_fila)


with open("Preguntas.csv", "w", encoding="utf-8", newline='') as archivo:
    escritor = csv.writer(archivo)
    escritor.writerow(nombres_columnas)
    for fila in matriz_sin_diccionario:
...
with open("Preguntas.csv", encoding="utf-8") as archivo:
    lector = csv.reader(archivo)
    next(lector)  # saltar la primera línea

    for fila in lector:
        if len(fila) == 9:  # solo si la fila está completa
            pregunta = {
                "pregunta": fila[0],
                "respuesta_1": fila[1],
                "respuesta_2": fila[2],
                "respuesta_3": fila[3],
                "respuesta_4": fila[4],
                "respuesta_correcta": int(fila[5]),
                "veces_preguntada": int(fila[6]),
                "aciertos": int(fila[7]),
                "fallos": int(fila[8])
            }
